...
class Kagome: # Ohgushi 00, PRB 62.6067
    def __init__(self,phi,t1=1,t2=None,t3=None):
        if t2 == None:
            t2 = t1
        if t3 == None:
            t3 = t2
        ...
        self.t3 = t3
    def hk(self,k1,k2):
        # k1 = k dot a1, k2 = k dot a2. Note that a3 = -(a1 + a2) (see p2 of
        # paper beneath FIG 1), so k dot a3 = -k1-k2
        #
        # note also: in the paper, a1,a2 and a3 are bonds between nearest neighbors hence all have only half the length of a basis vector. Correspondingly in the Hamiltonian below, k1,k2 -> k1/2, k2/2
        res = np.zeros((3,3),dtype=np.complex)

        phase = np.exp(-1j*self.phi/3)
        tvec = np.array((self.t1,self.t2,self.t3))
        res[0,1], res[1,2], res[2,0] = 2*tvec*np.cos((k1/2,k2/2,(-k1-k2)/2)) * phase
    ...
    def export_hk(self,nx=50,ny=50,fn='kagomi-erg'):
        with open(fn + '.par', 'w') as par:
            ...
            t3 = %g
            phi = %g
            """%(nx,ny,self.t1,self.t2,self.t3,self.phi)
            print(txt,file=par)
